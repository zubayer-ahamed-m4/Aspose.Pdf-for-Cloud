package AsposePdfCloud::ApiClient;
...
sub new
{
	...
	if(not defined $AsposePdfCloud::Configuration::app_sid or $AsposePdfCloud::Configuration::app_sid eq ''){
		croak("Aspose Cloud App SID key is missing.");
    }
	
	if(not defined $AsposePdfCloud::Configuration::api_key or $AsposePdfCloud::Configuration::api_key eq ''){
    ...
    my (%args) = (
        ...
        'base_url' => $AsposePdfCloud::Configuration::api_server,
        @_
    );
...
sub call_api {
	...
	$resource_path =~ s/\Q{appSid}\E/$AsposePdfCloud::Configuration::app_sid/g;
  
    my $_url = $self->{base_url} . $resource_path;
  
    # update signature
    my $signature = $self->sign($_url, $AsposePdfCloud::Configuration::app_sid, $AsposePdfCloud::Configuration::api_key);
    ...
    $self->{ua}->timeout($self->{http_timeout} || $AsposePdfCloud::Configuration::http_timeout); 
    $self->{ua}->agent($self->{http_user_agent} || $AsposePdfCloud::Configuration::http_user_agent);
...
sub deserialize
{
    ...
    if (not defined $data) {
        ...
        my $_instance = use_module("AsposePdfCloud::Object::$class")->new;
        if (ref $data eq "HASH") {
